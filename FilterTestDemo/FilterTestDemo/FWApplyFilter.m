...
+ (UIImage *)customfilter:(NSString *)str image:(UIImage *)image {
    GPUImageFilter *customFilter = [[GPUImageFilter alloc] initWithFragmentShaderFromFile:str];
    GPUImagePicture *pic = [[GPUImagePicture alloc] initWithImage:image];
    [pic addTarget:customFilter];
    [pic processImage];
    [customFilter useNextFrameForImageCapture];
    return [customFilter imageFromCurrentFramebuffer];
}

+ (UIImage *)snapchatFilter:(UIImage *)image {
    GPUImageSnapchatFilter *filter = [[GPUImageSnapchatFilter alloc] init];
    [filter forceProcessingAtSize:image.size];
    GPUImagePicture *pic = [[GPUImagePicture alloc] initWithImage:image];
    [pic addTarget:filter];
    [pic processImage];
...
+ (UIImage *)qmfilter:(UIImage *)image index:(NSInteger)index {
    ...
    NSLog(@"%@",path);
    NSArray *array =  [QMFilterModel buildFilterModelsWithPath:path];;
    NSLog(@"%@\n%ld",array,array.count);
    QMFilterModel *model= array[index];
    QMImageFilter *filter = [[QMImageFilter alloc]initWithFilterModel:model];
    GPUImagePicture *pic = [[GPUImagePicture alloc] initWithImage:image];
    [pic addTarget:filter];
    [pic processImage];
    [filter useNextFrameForImageCapture];
